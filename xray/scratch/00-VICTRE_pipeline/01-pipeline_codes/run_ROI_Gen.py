...
dataset_root_folder = os.path.expanduser('~/00-VICTRE_pipeline/05-cropped_set/')  ## the phantom dataset folder

def check_unprocessed_data(dataset_folder, folder_list = []):
    folder_list = glob.glob(os.path.join(dataset_folder,'*'))
#    patient_list = []
    updated_list = []
	...
	if os.path.isdir(folder):
	    #patient_list.append(np.int32(os.path.basename(folder)))
            ...
            proc_flag_file = os.path.join(folder,'logs','flag_ROI_gen.txt')
            if (not os.path.exists(proc_flag_file)) and (len(crop_files)>0):
                patient_list.append(np.int32(os.path.basename(folder)))
    return updated_list

patient_list = check_unprocessed_data(dataset_root_folder, [])  # initialize the to-be-processed random seed list

while(len(patient_list)>0):
    
    ## fetch a random seed
    fetch_seed = patient_list[0] # all ways choose the head of the list
    ## set a flag to occupy the data
    flag_file = os.path.join(dataset_root_folder,'{}'.format(fetch_seed),'flag_ROI_gen.txt')
    if os.path.exists(flag_file):
        patient_list = check_unprocessed_data(dataset_folder, patient_list)
        continue
    else:
    ...
    patient_list = check_unprocessed_data(dataset_root_folder, patient_list)
