...
cfg_folder = os.path.expanduser('/scratch/00-VICTRE_pipeline/00-base_input_files/02-mcgpu_files/cfg')
mcgpu_cmd_folder = os.path.expanduser('/scratch/04-VICTRE_MCGPU')
...
parser.add_argument('signal_model_code', type = int) # signal model code 
...
output_data_folder = os.path.join(patient_folder,'phantom_l{}'.format(signal_model_code),'mc_gpu')
generate_folder(output_data_folder)
...
# 	phantom_data_file = os.path.join(patient_folder,'pcl_{}_crop.raw.gz'.format(rand_seed))
	phantom_data_file = os.path.join(patient_folder,'phantom_l{}'.format(signal_model_code),'pcl_{}_crop.raw.gz'.format(rand_seed))
	mhd_file = os.path.join(patient_folder,'pc_{}_crop.mhd'.format(rand_seed))
...
if isInserted:
    log_file = os.path.join(log_folder, 'log_xray_insert_{}.txt'.format(signal_model_code))
    time_file = os.path.join(log_folder,'time_xray_insert_{}.txt'.format(signal_model_code))
else:
