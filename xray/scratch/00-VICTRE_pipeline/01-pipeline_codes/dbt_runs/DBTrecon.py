...
flatfield_projection_root_folder = os.path.expanduser('/scratch/xray_set/dbt_flatfield/flatfield')
FBP_cmd_folder = os.path.expanduser('/scratch/05-FBP_DBT_Recon')
concatenate_cmd_folder = os.path.expanduser('/scratch/05-FBP_DBT_Recon')
...
if isInserted:
    output_data_folder = os.path.join(patient_folder, 'phantom_l1', 'dbt_recon')
    generate_folder(output_data_folder)
    patient_data_folder = os.path.join(patient_folder, 'phantom_l1')
    mcgpu_files = 'pcl_{}_crop.raw.gz'.format(rand_seed)
    # mcgpu_folder = os.path.join(patient_folder,'phantom_l1','mc_gpu', mcgpu_files)
    # mcgpu_data_folder = os.path.join(patient_folder,'phantom_l1','mc_gpu')
else:
    output_data_folder = os.path.join(patient_folder, 'phantom_l0', 'dbt_recon')
    generate_folder(output_data_folder)
    patient_data_folder = os.path.join(patient_folder, 'phantom_l0')
    mcgpu_files = 'pc_{}_crop.raw.gz'.format(rand_seed)
    # mcgpu_folder = os.path.join(patient_folder,'phantom_l1','mc_gpu', mcgpu_files)
    # mcgpu_data_folder = os.path.join(patient_folder,'phantom_l0','mc_gpu', mcgpu_files)

mcgpu_data_folder = os.path.join(patient_data_folder,'mc_gpu',mcgpu_files)
...
# copyfile(os.path.join(patient_data_folder,'mc_gpu','{}_3000x1500pixels_25proj.raw'.format(mcgpu_files)),os.path.join(output_data_folder,'DBT_{}_25proj.raw'.format(rand_seed))) ## copy the concatenated file to the output folder, so that the reconstruction codes can run
move(os.path.join(patient_data_folder,'mc_gpu','{}_3000x1500pixels_25proj.raw'.format(mcgpu_files)),os.path.join(output_data_folder,'DBT_{}_25proj.raw'.format(rand_seed)))
...
if not os.path.exists(os.path.join(output_data_folder,'flatfield_25proj.raw')):
    ...
    mcgpu_folder = os.path.join(patient_data_folder,'mc_gpu')
...
myCmd = '{0:}/FBP_DBTrecon 25 3000 1500 0.0085 65.0 60.0 0.000 50.00 {1:} {2:} {3:} 0.0050 0.0085 0.1 0 -25.0 2.083333333333333333 {4:} {5:}'.format(FBP_cmd_folder,dim_y, dim_x, dim_z, rand_seed, output_data_folder)
print(myCmd)
#os.system(myCmd)
...
## clearn the intermediate data
# remove concatenated mcgpu files
# concatenated_file = os.path.join(patient_data_folder,'mc_gpu','{}_3000x1500pixels_25proj.raw'.format(mcgpu_files))
dbt_input_file = os.path.join(output_data_folder,'DBT_{}_25proj.raw'.format(rand_seed))
if os.path.exists(dbt_input_file):
    os.remove(dbt_input_file)

# remove the original DBT data
dbt_flatfield_file = os.path.join(output_data_folder,'flatfield_25proj.raw')
if os.path.exists(dbt_flatfield_file):
    os.remove(dbt_flatfield_file)

# remove the platfield files
