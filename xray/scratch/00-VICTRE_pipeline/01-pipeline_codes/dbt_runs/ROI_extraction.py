...
# roi_cmd_folder = os.path.expanduser('/shared/turing/Phantom/00-VICTRE_pipeline/00-base_input_files/04-other_shared_files/ROIExtraction')
roi_cmd_folder = os.path.expanduser('/scratch/00-VICTRE_pipeline/01-pipeline_codes/dbt_runs/roi_extraction_cmd')
template_flatfield_folder = os.path.expanduser('/scratch/xray_set/dbt_flatfield/flatfield')
...
parser.add_argument("signal_model_code", type = int) # signal type: SP-> signal present or SA-> signal absent
...
output_data_folder = os.path.join(patient_folder, 'phantom_l{}'.format(signal_model_code), 'roi_voi')
...
print(template_flatfield_folder)
flatfield_files = glob.glob(src_flatfield_ptrn)
print(flatfield_files)
...
trg_flatfield_file = os.path.join(patient_folder, 'phantom_l{}'.format(signal_model_code), 'mc_gpu/flatfield_{0:}_crop.raw.gz_0000.raw'.format(rand_seed))
...
if image_type == 'dbt':
    ...
    dbt_size_file = os.path.join(patient_folder, 'phantom_l{0:}'.format(signal_model_code),'dbt_recon','DBT_recon_size.txt')
    nx, ny, nz = 0, 0 ,0
    val_list = []
	...
	trg_DBT_flatfield_file = os.path.join(patient_folder, 'phantom_l{}'.format(signal_model_code), 'dbt_recon/flatfield_25proj.raw')
...
if image_type == 'mammo' and signal_case == 'SA':
	...
	mcgpu_image_folder = os.path.join(patient_folder,'phantom_l{}'.format(signal_model_code), 'mc_gpu')
	...
	generate_folder(mass_output_folder)
	generate_folder(loc_output_folder)
	log_file = os.path.join(log_folder, 'log_mammo_SP_{}.txt'.format(signal_model_code))
	time_file = os.path.join(log_folder,'time_mammo_SP_{}.txt'.format(signal_model_code))
	extract_cmd = 'python2 {0:}/roiExtraction_mammo_SP.py {1:} 0 60.25 630 0 {2:} 0.085 {3:} {4:} {5:} 0 0 0 {6:} {7:} {8:} 65 109 1500 3000 -20 {9:} {10:} {11:} {12:} {13:} {14:}'.format(roi_cmd_folder, rand_seed, elementSpac, offsetX, offsetY, offsetZ, dimX, dimY, dimZ, phantom_folder, mcgpu_image_folder,flatfield_file_folder,cal_output_folder, mass_output_folder, loc_output_folder)

elif image_type == 'dbt' and signal_case == 'SA':
	## DBT ROV extraction for SA case
	phantom_folder = patient_folder
	recon_image_folder = os.path.join(patient_folder, 'phantom_l{}'.format(signal_model_code), 'dbt_recon')
	...
	move(dbt_data_file, input_data_file)
	log_file = os.path.join(log_folder, 'log_DBT_SA.txt')
	time_file = os.path.join(log_folder,'time_DBT_SA.txt')
	extract_cmd = 'python2 {0:}/roiExtraction_DBT_FBP_SA.py {1:} 0 60.25 630 0 {2:} {3:} {4:} 0.085 1 {5:} {6:} {7:} 0 0 0 65 109 5 9 2 2 4 4 {8:} {9:} {10:} {11:} {12:} {13:}'.format(roi_cmd_folder, rand_seed,
			nx, ny, nz, offsetX, offsetY, offsetZ, elementSpac, phantom_folder, recon_image_folder, cal_output_folder, mass_output_folder, loc_output_folder)
	...
	# copyfile(dbt_data_file, input_data_file)
	move(dbt_data_file, input_data_file)
	extract_cmd = 'python2 {0:}/roiExtraction_DBT_FBP_SP.py {1:} 0 60.25 630 0 {2:} {3:} {4:} 0.085 1 {5:} {6:} {7:} 0 0 0 65 109 5 9 2 2 4 4 {8:} {9:} {10:} {11:} {12:} {13:}'.format(roi_cmd_folder, rand_seed,
			nx, ny, nz, offsetX, offsetY, offsetZ, elementSpac, phantom_folder, recon_image_folder, cal_output_folder, mass_output_folder, loc_output_folder)
	log_file = os.path.join(log_folder, 'log_DBT_SP_{}.txt'.format(signal_model_code))
	time_file = os.path.join(log_folder,'time_DBT_SP_{}.txt'.format(signal_model_code))
